...
def compare(w, X_test, Y_test):
    ...
    :param Y_test: classifications of test set.
    ...
    print("Loss rate: %f" % (100 * (loss / len(X_test))))
    print("Success rate: %f" % (100 - 100 * (loss / len(X_test))))
    return results
...
def run(train_x_file, train_y_file, test_x_file):
    ...
    # read the classifications file for test
    with open('test_y.txt') as test_y_file:
    ...
    # normalize the data
    X_train_perc, X_test_perc = z_score_norm_data(X_train_perc, X_test_perc)
    X_train_svm, X_test_svm = z_score_norm_data(X_train_svm, X_test_svm)
    X_train_pa, X_test_pa = min_max_norm_data(X_train_pa, X_test_pa)
    ...
    perc_list = compare(w1, X_test_perc, Y_test)
    # perc_list = classify(w, X_test)

    # w = create_weights(X_train, Y_train)
    w2 = svm_predict(w2, X_train_svm, Y_train)
    svm_list = compare(w2, X_test_svm, Y_test)
    # svm_list = classify(w, X_test)

    # w = create_weights(X_train, Y_train)
    w3 = pa_predict(w3, X_train_pa, Y_train)
    pa_list = compare(w3, X_test_pa, Y_test)
    # pa_list = classify(w, X_test)

    return perc_list, svm_list, pa_list

def main():
    train_x, train_y, test_x = sys.argv[1:]
    with open(train_x) as f1:
        with open(test_x) as f2:
